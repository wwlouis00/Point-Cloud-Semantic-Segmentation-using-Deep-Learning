...
# 地址
# 1\读取的ply地址
labelpath = '/home/mspl/kitti_semantics/test/Log_2022-10-16_16-15-47_fulltrain/test_preds'
inputpath = 'Dataset/kitti/original_block_ply'
labelpaths = listdir(labelpath)
inputpaths = listdir(inputpath)
labelpaths.sort()
inputpaths.sort()
inputlabelpath = ['/home/mspl/kitti_semantics/test/Log_2022-10-19_14-29-04_notfinetuned/test_preds/instance3_2.label']#[join(labelpath, f) for f in labelpaths if isfile(join(labelpath, f))]
inputplypath = ['/home/mspl/kitti_semantics/Dataset/kitti/original_block_ply/instance3_2.ply']#[join(inputpath, f) for f in inputpaths if isfile(join(inputpath, f))]
outputpath = '/home/mspl/kitti_semantics/test/Log_2022-10-19_14-29-04_notfinetuned/test_preds2'
outputplypath = [join(outputpath, 'instance3_2.ply')]#[join(outputpath, f) for f in inputpaths if isfile(join(inputpath, f))]
...
for i in tqdm(range(len(inputplypath))):
    ...
    # print(rgb)
    write_ply(outputplypath[i], [xyz, rgb, label], ['x', 'y', 'z', 'red', 'green', 'blue', 'semantic'])  # xuek [xyz, rgb, label]
